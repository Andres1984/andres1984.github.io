...
* Desarrollar competencias académicas implementando conceptos teóricos en escenarios reales
...
p2 <- ggdraw() + draw_image("/Users/andresmartinez/Documents/GitHub/andres1984.github.io/portfolio.github.io/Jim_Simons_at_MSRI.jpg", scale = 0.9)
...
library(quantmod)
si=c("BA","F","MSFT","MCD")
getSymbols(si,src="yahoo",from="2020-01-20",to="2021-06-20")
```
...
library(quantmod)
si=c("BA","F","MSFT","MCD")
getSymbols(si,src="yahoo",from="2018-03-24",to="2019-10-1")
# sin COVID
a=Delt(BA$BA.Close)[-1]
b=Delt(F$F.Close)[-1]
c=Delt(MSFT$MSFT.Close)[-1]
...
p<- plot_ly( x = final_df$sig.z, y = final_df$mu.z, type="scatter",name = "Frontera Eficiente")%>% 
  add_trace(x = final_df$vol, y = final_df$rend,name = final_df$Acciones)%>%      add_trace(x = final_df$sig.pe, y = final_df$mu.pe,name="Indice de Sharpe") %>% layout(title="Portafolio")
